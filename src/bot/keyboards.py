...
def onboarding_start_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.add(InlineKeyboardButton(
        text="Понятно, готов дать первую ссылку",
...
INTERVAL_CHOICES: list[tuple[str, int, int]] = [
    ("15–20 минут", 15 * 60, 20 * 60),
    ("20–30 минут", 20 * 60, 30 * 60),
    ("30–60 минут", 30 * 60, 60 * 60),
]
...
def main_menu_keyboard(is_paused: bool) -> InlineKeyboardMarkup:
    ...
    builder.add(InlineKeyboardButton(text="🔄 Последние объявления", callback_data="menu:last_n"))
...
def choose_subscription_for_lastn_keyboard(
    subscriptions: list[tuple[int, str, str]],
) -> InlineKeyboardMarkup:
    """
    Меню -> "Последние N объявлений" -> если подписок несколько, нужно выбрать
    по какой именно. Отдельная от choose_subscription_keyboard (та — для
    редактирования ссылки) с собственным callback-префиксом ("lastn:sub:" vs
    "editlink:sub:") — иначе один и тот же колбэк ловился бы не тем
    обработчиком в зависимости от того, какой роутер зарегистрирован раньше.
    """
    site_labels = {"ss_ge": "ss.ge", "myhome_ge": "myhome.ge"}
    ...
    for subscription_id, site, filter_url in subscriptions:
        ...
        builder.add(InlineKeyboardButton(
            text=f"{site_label}: {short_url}",
            callback_data=f"lastn:sub:{subscription_id}",
        ))
    builder.add(InlineKeyboardButton(text="Отмена", callback_data="lastn:cancel"))
    builder.adjust(1)
    return builder.as_markup()
 
 
LASTN_HOUR_CHOICES: list[tuple[str, int]] = [
    ("1 час", 1),
    ("3 часа", 3),
    ("6 часов", 6),
    ("12 часов", 12),
...
def lastn_hours_keyboard() -> InlineKeyboardMarkup:
    """Меню -> "Последние объявления" -> выбор периода. Кнопками, не свободным
    текстом — быстрее тапнуть на телефоне, и не нужна валидация ввода."""
    builder = InlineKeyboardBuilder()
    for label, hours in LASTN_HOUR_CHOICES:
        builder.add(InlineKeyboardButton(text=label, callback_data=f"lastn:hours:{hours}"))
    builder.add(InlineKeyboardButton(text="Отмена", callback_data="lastn:cancel"))
    builder.adjust(1)
    return builder.as_markup()
