...
async def send_listing(
    ...
    само сообщение-носитель кнопок (его message_id к моменту нажатия кнопки
    уже известен — это callback.message — в отличие от момента создания
    клавиатуры, см. db_schema.sql: listing_deliveries). Носитель тоже
    именно УДАЛЯЕТСЯ, а не редактируется в подтверждение — Telegram не
    разрешает editMessageText на сообщении типа Location (если носитель —
    карта), поэтому единообразно удаляем все части через delete_message."""
    ...
    #
    # ВАЖНО: text здесь НЕ может быть пустой строкой или невидимым юникод-
    # символом — на практике Telegram отклоняет как "text must be non-empty"
    # и \u200b (zero-width space), И \u2800 (BRAILLE PATTERN BLANK), хотя
    # обе формально не пустые строки в Python — сервер Telegram, похоже,
    # применяет более широкую нормализацию/trim, чем стандартный
    # str.isspace(). Больше не пытаемся обмануть это невидимым символом —
    # используем короткий ВИДИМЫЙ маркер (одна точка), который не может быть
    # схлопнут никакой нормализацией ни при каких обстоятельствах.
...
async def delete_listing_handler(callback: CallbackQuery, db_conn, bot: Bot) -> None:
    ...
    # Сообщение-носитель кнопок удаляем ТАК ЖЕ, как и остальные — раньше оно
    # редактировалось (edit_text) вместо удаления, но это ломалось, когда
    # носителем была карта (send_location): Telegram НЕ разрешает
    # editMessageText на сообщении типа Location — "message can't be edited"
    # (подтверждено на реальном случае). Само собой, callback.message —
    # это и есть носитель, его message_id уже известен на этом этапе (в
    # отличие от момента создания клавиатуры, где он ещё не существовал).
    try:
