...
-- объявления. Сообщение-носитель кнопки сюда НЕ входит не потому, что оно
-- не удаляется (удаляется точно так же, через delete_message), а потому что
-- его message_id ещё не существует в момент создания клавиатуры/записи —
-- к моменту нажатия кнопки он уже известен через сам callback (callback.message).
